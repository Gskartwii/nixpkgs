...
bash.runCommand "${pname}-${common.version}"
  ...
  (
    ...
    + lib.optionalString (buildPlatform.config != hostPlatform.config) ''
      ...
      export AS_FOR_TARGET="${lib.getExe' binutils-hosttarget "${binutilsTargetPrefix}as"}"
      export LD_FOR_TARGET="${lib.getExe' binutils-hosttarget "${binutilsTargetPrefix}ld"}"
    ''
    + ''
      popd
      ...
      bash ../src/configure \
        ...
        --with-build-time-tools=${binutils-buildhost}/bin \
        --enable-fast-install \
